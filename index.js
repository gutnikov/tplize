...
function type( obj ) {
	...
	return isObj || isFn ? class2type[toString.call( obj ) ] || "object" : typeof obj;
}
...
( function() {
	...
	var inputs = ( "text password checkbox radio button submit reset " +
		"file hidden image datetime datetime-local " +
		"date month time week number range email url search tel color" ).split( " " );

	var tags = ( "a button command abbr address area article aside " +
		...
		"reset file hidden image datetime datetime-local " +
		"date month time week number range email url search tel color" ).split( " " );
